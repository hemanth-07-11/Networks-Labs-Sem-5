...
def print_choices():
    os.system("lINK STATE ROUTING PROTOCOL")
    print("\n######################################################\n")
...
def check_choices(command):
    if not command.isdigit():
        ...
        if command > 4 or command < 1 :
            print("Enter a valid command from given choices..")
            return -1
        else:
            return command
           ...
           if matrix_set == 0 or answer == 'Y' or answer == 'y':
            ...
            if matrix_set == 1 :
                ...
                if start.isdigit() and int(start) > 0 and int(start) <= len(router_matrix):
                    ...
                    start = 0
                    print("\nEnter a valid source router.")
            else:
                print("\nNo network topology matrix exist. Upload the data file first.. ")
        elif command == 3:
            if matrix_set == 1 :
                end = input("\nSelect a destination router : ")
                if end.isdigit() and int(end) > 0 and int(end) <= len(router_matrix):
                    if int(start) == 0:
                        print("\nNo source router selected yet. Select a source router using choice : 2.")
                    elif int(start) == int(end):
                        print("\nSource and Destination routers are same. Select a different destination router.")
                    ...
                    print("\nEter a valid destination router.")
                pass
            else :
